	...
	if(alerts.length){
		alerts.forEach(function(_alert){
			if(
				...
				var el = _alert.querySelector(".js-ajax-form-alert-text");
				el.innerText = text;
...
var button1 = function () {
	render(setTest, document.querySelector('#test1-render'))
};

var setTest = function () {
    if (data.testid == 'some text') {
        template = '<p>some text from js</p>';
    } else {
        template = '<p>other message from js</p>';
    }
    return template;
};
...
var function1 = function(id){
	document.querySelectorAll(".tab-valores").forEach(tab => {
		tab.style.display = "none";
	});
	document.querySelectorAll(".svg-valores").forEach(svg => {
		svg.setAttribute("fill", "#fff");
	})
	const svg = document.querySelector("#test-svg-id");
	const selected_svg = document.querySelector("#svg-valores-" + String(id));
	const element = document.querySelector("#valor-" + String(id));
	function2(element, selected_svg);
};

var function2 = function(element, svg){
	element.style.display = "block";	
	svg.setAttribute('fill', '#FE0048')
};